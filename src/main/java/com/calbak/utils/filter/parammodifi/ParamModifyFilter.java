package com.calbak.utils.filter.parammodifi;
...
import static com.calbak.utils.filter.Util.cookieToMap;
...
public class ParamModifyFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            if ("local".equals(profile) || "dev".equals(profile)) { // 복호화가 필요 없는 경우의 add() 메소드 사용
            ...
            if (!"local".equals(profile) && !"dev".equals(profile)) { // 복호화가 필요한 경우의 decodeAdd() 메소드 사용
