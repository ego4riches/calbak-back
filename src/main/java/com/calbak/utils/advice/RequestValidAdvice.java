package com.calbak.utils.advice;


import com.calbak.utils.advice.dto.DBError;
import com.calbak.utils.advice.dto.Error;
import com.calbak.utils.exception.ConditionException;
import com.calbak.utils.response.ErrorResponse;
...
public class RequestValidAdvice {
    ...
    public ResponseEntity<ErrorResponse> multipartException(MultipartException e, HttpServletRequest httpServletRequest) {
        ...
        if (e.getCause().getCause() instanceof SizeLimitExceededException exception) {
        ...
        } else if (e.getCause().getCause() instanceof InvalidContentTypeException) {
            /* 콘텐츠 타입 설정 에러 */

            log.error("[ERROR] [UNSUPPORTED MEDIA TYPE] {}", e.getCause().getCause().getClass().getName());
            log.error("잘못된 Content-type 으로 요청 하셨습니다.");
            log.error("[ERROR]");
            ...
            return new ResponseEntity<>(
                    ErrorResponse.builder()
                            ...
                            .info(List.of(
                                    Error.builder()
                                            .message("Content-Type이 잘못되었습니다.")
                                            .key("Content-Type")
                                            .value(e.getCause().getCause().getMessage())
                                            .build()
                            ))
                            .build()
                    , HttpStatus.UNSUPPORTED_MEDIA_TYPE);
